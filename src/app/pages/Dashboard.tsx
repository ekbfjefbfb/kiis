...
export default function Dashboard() {
  ...
  return (
    <div className="h-[100dvh] w-full bg-black text-white font-sans overflow-hidden flex flex-col relative selection:bg-white/20">
      <header className="px-6 pt-12 pb-6 flex justify-between items-end border-b border-zinc-800 bg-black/80 backdrop-blur-xl sticky top-0 z-20 shrink-0">
        <div>
        ...
        <div className="flex gap-3">
          <Link to="/calendar" className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center active:scale-90 transition-transform">
            <Calendar size={18} className="text-zinc-400" />
          </Link>
          <Link to="/profile" className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center active:scale-90 transition-transform">
            <User size={18} className="text-zinc-400" />
          </Link>
        </div>
      </header>

      <main className="flex-1 overflow-y-auto scrollbar-hide px-5 pt-8 max-w-2xl mx-auto w-full pb-20 space-y-12">
        
        <section className="grid grid-cols-1 gap-4">
          <Link to="/live" className="block group">
            <motion.div whileTap={{ scale: 0.98 }} className="bg-zinc-900 border border-zinc-800 rounded-[32px] p-6 flex items-center justify-between transition-all active:bg-zinc-800 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-1 h-full bg-white/10" />
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center shrink-0 border border-zinc-800">
                  <Radio size={24} className="text-white" />
                </div>
                <div className="text-left">
                  <p className="text-lg font-bold uppercase italic leading-none tracking-tight text-white">Grabar_Ahora</p>
                  <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-widest mt-2 leading-relaxed">Captura IA instantánea</p>
                </div>
              </div>
          ...
          <Link to="/chat" className="block">
            <motion.div whileTap={{ scale: 0.98 }} className="bg-zinc-900 border border-zinc-800 rounded-[28px] p-5 flex items-center justify-between transition-all active:bg-zinc-800 relative overflow-hidden">
              <div className="flex items-center gap-5">
                <div className="w-10 h-10 rounded-xl bg-zinc-800 flex items-center justify-center border border-zinc-800 shadow-inner">
        ...
        <section className="space-y-6">
          <div className="flex justify-between items-center px-2 text-left">
            ...
            <button onClick={() => setIsAddingClass(true)} className="w-9 h-9 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500 active:scale-90 active:bg-zinc-800 transition-all shadow-lg">
              <Plus size={16} />
            </button>
          </div>
          <div className="grid grid-cols-1 gap-3">
            {CLASSES.map((cls) => (
              <Link key={cls.id} to={`/class/${cls.id}`} className="block group">
                <motion.div whileTap={{ scale: 0.99 }} className="bg-zinc-900 border border-zinc-800 rounded-[32px] p-5 flex items-center gap-5 active:bg-zinc-800 transition-all text-left relative overflow-hidden group">
                  <div className="absolute top-0 left-0 w-1 h-full bg-white/[0.03]" />
                  <div className="w-10 h-10 rounded-xl bg-zinc-800 border border-zinc-800 flex items-center justify-center shrink-0 shadow-inner">
        ...
        <section className="space-y-6">
          ...
          {recentNotes.length === 0 ? (
            <div className="bg-zinc-900 border border-dashed border-zinc-800 rounded-[32px] p-10 text-center space-y-4 opacity-20">
            ...
            <div className="space-y-3">
              {recentNotes.slice(0, 2).map((n) => (
                <Link key={n.id} to={`/note/${n.id}`} className="block group">
                  <motion.div whileTap={{ scale: 0.98 }} className="bg-zinc-900 border border-zinc-800 rounded-[24px] p-5 flex items-center justify-between transition-all active:bg-zinc-800 text-left">
