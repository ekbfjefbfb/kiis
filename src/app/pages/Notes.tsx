...
import { 
  ArrowLeft, Search, Plus, ChevronRight, FileText, Clock
} from "lucide-react";
    ...
    <div className="h-[100dvh] w-full bg-black text-white font-sans selection:bg-white/30 overflow-hidden flex flex-col relative">
      <header className="px-[env(safe-area-inset-left,1.5rem)] pr-[env(safe-area-inset-right,1.5rem)] pt-[max(env(safe-area-inset-top,2rem),3rem)] pb-6 flex justify-between items-end border-b border-white/5 bg-black/80 backdrop-blur-xl sticky top-0 z-20 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[9px] font-medium text-zinc-600 uppercase tracking-[0.3em] mb-1 text-left">Archivo_</p>
            <h1 className="text-xl font-bold uppercase italic tracking-tighter leading-none text-white">Mis Notas</h1>
          </div>
        </div>
        <button 
        ...
        {loading ? (
          ...
          filteredNotes.map((note) => (
            <Link key={note.id} to={`/note/${note.id}`} className="block group">
              <motion.div 
                whileTap={{ scale: 0.98 }}
                className="bg-zinc-900/40 border border-white/5 rounded-[28px] p-5 flex items-center justify-between transition-all active:bg-zinc-800"
              >
                <div className="flex items-center gap-5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[9px] font-bold text-zinc-700 uppercase tracking-widest mb-1">
                      {new Date(note.created_at || Date.now()).toLocaleDateString()}
                    </p>
                    <p className="text-base font-bold uppercase italic tracking-tight text-white truncate leading-none mb-1.5">
                      {note.title || "Nota"}
                    </p>
                    <p className="text-[10px] text-zinc-500 truncate italic line-clamp-1">
