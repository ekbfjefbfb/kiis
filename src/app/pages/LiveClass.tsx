...
export default function LiveClassPage() {
  ...
  return (
    <div className="h-[100dvh] w-full bg-black text-white font-sans overflow-hidden flex flex-col relative selection:bg-white/20">
      {/* Background Real-time Transcription - Brutal Clarity */}
      <div 
        ref={scrollRef}
        className="absolute inset-0 px-8 pt-48 pb-60 overflow-y-auto scrollbar-hide opacity-[0.08] pointer-events-none z-0"
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black uppercase italic tracking-tighter leading-none">
              {selectedClass ? selectedClass.name : "Nueva Sesión"}
            </h1>
            <p className="text-[9px] font-bold text-white/30 uppercase tracking-[0.2em] mt-1.5">
              {isRecording ? "Capturando_Transcribiendo" : "Grabación Inteligente"}
            </p>
          </div>
        </div>
        {selectedClass && !isRecording && (
          <button onClick={() => setShowClassPicker(true)} className="px-4 py-2 rounded-full bg-zinc-900 border border-white/10 text-[10px] font-black uppercase tracking-widest text-zinc-400 active:scale-95">
            Cambiar
          </button>
        )}
      </header>

      {/* Selector de Materia - Inmersivo */}
      <AnimatePresence>
        {showClassPicker && (
          <motion.div 
            ...
            <div className="space-y-3 overflow-y-auto scrollbar-hide flex-1">
              {CLASSES.map((cls) => (
                <button 
                  ...
                  className="w-full p-6 rounded-[28px] bg-zinc-900/40 border border-white/5 flex items-center justify-between text-left active:bg-zinc-800 transition-all"
                >
                  <div>
        ...
        <AnimatePresence mode="wait">
          {!sessionResult && !isProcessing ? (
            ...
            >
              <div className="relative">
                ...
                <motion.button
                  ...
                  className={`w-40 h-40 rounded-[56px] flex items-center justify-center border-2 transition-all duration-700 shadow-2xl z-10 relative ${isRecording ? 'bg-white border-white' : 'bg-zinc-900 border-white/10'}`}
                >
              ...
              <div className="text-center space-y-4">
                <h2 className="text-4xl font-black uppercase italic tracking-tighter leading-none">
                  {isRecording ? "Capturando" : "Empezar"}
                </h2>
                <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.4em] max-w-[240px] mx-auto leading-relaxed">
                  {isRecording ? "Transcripción activa en segundo plano. No cierres la app." : "Convierte la voz del profesor en notas inteligentes."}
                </p>
              </div>
            </motion.div>
          ) : isProcessing ? (
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              className="space-y-12 pb-48"
            >
              {/* Resumen de Clase */}
              <section className="space-y-6">
                <div className="flex items-center gap-3 ml-2">
                  <div className="w-2 h-2 bg-white rounded-full" />
                  <h3 className="text-[12px] font-black uppercase tracking-[0.5em] text-white/30">Síntesis</h3>
                </div>
              ...
              <section className="space-y-6">
                <div className="flex items-center gap-3 ml-2">
                  ...
                  <h3 className="text-[12px] font-black uppercase tracking-[0.5em] text-white/30">Tareas_Agenda</h3>
                </div>
                <div className="space-y-3">
                  {sessionResult.tasks.map((task) => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      className={clsx(
                        "w-full p-7 rounded-[32px] border flex items-center justify-between transition-all duration-500",
                        task.completed 
                    ...
                    >
                      <div className="flex items-center gap-6">
                        <div className={clsx(
                          ...
                          task.completed ? "bg-emerald-500 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.5)]" : "border-white/10"
                        )}>
                          {task.completed && <CheckCircle2 size={18} className="text-black" />}
                        </div>
                        <div className="text-left">
                          <p className={clsx(
                            "text-lg font-black uppercase italic transition-all tracking-tighter",
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest text-white/20">{task.dueDate}</p>
                          </div>
                        </div>
                      </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="w-full h-20 bg-white text-black rounded-[40px] font-black uppercase italic tracking-tighter text-xl shadow-[0_0_50px_rgba(255,255,255,0.15)] flex items-center justify-center gap-4 active:bg-zinc-200 transition-colors"
              >
